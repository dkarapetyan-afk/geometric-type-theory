...
Counted from those tensors, including the final RMSNorm and the untied output head, the published model has 46,702,792,704 parameters, of which 12,879,925,248 are active at top-2. `gtt compile` checks that count. The memory parameter has two budgets. Code memory holds the kernel images a stage launches (`gemm`, `gemm_dgrad`, `gemm_wgrad`, `rmsnorm_fwd`, `rmsnorm_bwd`, `rope`, `softmax_fwd`, `softmax_bwd`, `silu_fwd`, `silu_bwd`, `embed`, `embed_bwd`, `ce_grad`). Each image size is the sm_75 cubin `.text` plus that kernel's constant banks. Buffer memory holds four kinds of data: checkpoints, the stage's parameter tensors, the adjoint of those parameters, and the scratch tape. A stage loads only its kernels and its buffers, runs the forward slice and the cotangent, writes `θ ← θ − η ∇L` back, and drops both. An expert stage does not keep the attention kernels resident, so the code peak is smaller than the sum of every kernel. Under 8 GiB of buffers and 1 MiB of code, with a microbatch of one sequence of 512 tokens, the 8x7B weights are staged one attention block or one expert at a time. A 1 GiB buffer budget cannot hold a single expert's weights and adjoint together, and a 64-byte code budget cannot hold one kernel image. `mixtral_cluster` names the heterogeneous target: `cpu0` has 64 GiB of host buffer and runs the embedding and the loss, `gpu0` (8 GiB) and `gpu1` (24 GiB) run attention and take turns holding experts, and `gpu2` (256 MiB) stays idle because no stage's buffers fit there. Each stage records its device, and the code size is the 4 KiB host entry on a CPU or the GPU cubin on a GPU. The links are a star through `cpu0`. The schedule is a sequence of forward runs, adjoint runs, and local parameter updates, with a message inserted wherever a tensor's producer and consumer sit on different devices. A GPU-to-GPU expert partial or adjoint is routed `gpu → cpu0 → gpu` because that is the only path. A device that meets the memory minimum for every stage it is given can run the pipeline; a missing link is a schedule error even when the bytes fit. `run_on_cluster` runs that schedule. Each device is a member: it binds a data socket and connects to the coordinator. The coordinator keeps those connections as the control plane, sends each member the others' data addresses, and walks the steps in order. A run executes that stage's forward or adjoint on the owning member, on the host tape, and the member keeps the tensors it produces. A send is a control message to the source; the source relays the named tensor along the scheduled path, and each hop waits for the next hop's acknowledgement, so the coordinator's acknowledgement means the destination inbox holds the payload. An update applies `θ ← θ − η ∇L` on the member that owns the parameters and writes those tensors back to the checkpoint directory. The demonstration checkpoint is a directory of raw tensors. The coordinator initializes it, and the members write the updated parameters. On `mixtral_cluster` that step includes a multi-hop relay through `cpu0`, and both that star and a single device match the single-buffer loss and the updated parameters. The runnable net is the same block at demonstration width. On the smooth configuration every expert is selected, so the loss is differentiable, and the adjoint matches a central difference on the embedding, an attention matrix, an expert down-projection, the router, and the output head. One SGD step, `θ ← θ − η ∇L`, lowers the loss. On the sparse configuration, top-2 of 8, an idle expert’s parameter gradient is zero and a routed expert’s is not.
...
`src/mixtral.rs` is the Mixtral model. `src/ad.rs` is the tape: forward values and the reverse adjoint. `src/cuda_emit.rs` prints the kernels. `src/stage.rs` places stages on a cluster and orders the messages. `src/dist.rs` is the coordinator and the members. `gtt compile` runs the demonstration and writes `examples/mixtral/kernels.cu`. `cargo test --offline --test mixtral` checks the parameter count, the finite-difference adjoint, the idle-expert gradient, the cluster schedule, the distributed step, and the GPU kernels.
